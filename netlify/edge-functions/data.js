...
//
// 2025-07-21 — adds `safeJson()` wrapper + headers + retry for every Binance call
// Bloc layout:
//   A  trend / volatility
//   B  price-action insight
//   C  volume-flow (15 m, 1 h, 4 h, 24 h)
//   D  derivatives positioning
//   E  sentiment
//   F  macro context
//
// All endpoints are public / no-key.

export default async (_req) => {
  const SYMBOL = 'BTCUSDT';               // change / loop for other coins
  ...
  /* ───────── resilient JSON fetch for Binance endpoints ───────── */
  async function safeJson(url, retries = 3) {
    for (let i = 0; i < retries; i++) {
      ...
      if (!ct.includes('application/json')) {          // got HTML / empty
  ...
  try {
    ...
    for (const tf of frames) {
      ...
      else {                // tight range → possible reversal setups
        const lastTwo=closes.slice(-2);
        note = lastTwo[1]>lastTwo[0]
          ? '↗ base forming – early long reversal?'
          : '↘ top forming – early short reversal?';
      }
  ...
  try {
    ...
    if(up==null) throw new Error('missing sentiment');
