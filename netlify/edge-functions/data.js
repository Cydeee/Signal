...
export default async (request) => {
  ...
  for (const tf of ['15m','1h','4h','1d']) {
    try {
      const rows = await fetch(`https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=${tf}&limit=${LIMIT}`)
        .then(r=>r.json());
  ...
  try {
    ...
    const [oiN, oiH] = await Promise.all([
      ...
      fetch(`https://api.binance.com/api/v3/openInterestHist?symbol=${SYMBOL}&period=1h&limit=24`).then(r=>r.json())
    ]);
  ...
  // BLOCK E
  try {
    const ge = await fetch('https://api.coingecko.com/api/v3/global').then(r=>r.json()).then(j=>j.data);
    result.dataE = {
      totalMcapT:   +((ge.total_market_cap.usd/1e12).toFixed(2)),
      mcap24hPct:   +ge.market_cap_change_percentage_24h_usd.toFixed(2),
      btcDominance: +ge.market_cap_percentage.btc.toFixed(2),
      ethDominance: +ge.market_cap_percentage.eth.toFixed(2),
    };
  } catch(e) {
