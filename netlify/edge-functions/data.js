export default async (_req) => {
  ...
  /* ───────── resilient fetch, but no payload changes ───────── */
  async function safeJson(url, retries = 3) {
    for (let i = 0; i < retries; i++) {
      ...
      let obj;
      try { obj = JSON.parse(txt); } catch { await new Promise(r=>setTimeout(r,400)); continue; }

      // Retry if Binance sent an error JSON (code < 0)
      if (obj && typeof obj.code === 'number' && obj.code < 0) { await new Promise(r=>setTimeout(r,400)); continue; }
      return obj;                    // ✅ good JSON (array or normal object)
    }
  ...
  /* ───────── BLOC A – price / volatility / trend (original logic) ───────── */
  ...
  /* ───────── BLOC D – derivatives positioning (unchanged logic) ───────── */
  try {
  ...
  /* ───────── BLOC F – macro context (unchanged) ───────── */
  try {
