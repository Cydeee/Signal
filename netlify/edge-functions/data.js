...
export default async (request) => {
  const SYMBOL = 'BTCUSDT';   // change to any futures symbol you need
  const LIMIT  = 250;         // klines per timeframe for bloc A

  const result = {
    dataA: {},   // price / TA
    dataB: null, // derivatives positioning
    dataC: {},   // liquidations
    dataD: null, // sentiment
    dataE: null, // macro context
    errors: []
  };
  ...
  try {
    ...
    const pct24h = (
      ((+oiNow.openInterest - +oiHist[0].sumOpenInterest) / +oiHist[0].sumOpenInterest) *
      100
    ).toFixed(1);
  ...
  try {
    ...
    for (const [lbl, hrs] of Object.entries(windows)) {
      ...
      url.searchParams.set('autoCloseType', 'LIQUIDATION');
      ...
      if (!ct.includes('application/json')) {
        result.errors.push(`C[${lbl}]: unexpected content-type: ${ct}`);
        dataC[lbl] = { long: 0, short: 0, total: 0 };
        continue;
      }
